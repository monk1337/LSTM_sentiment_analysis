...
class LSTM_Network(object):
    ...
    def __init__(self, vocab_size_, dropout_value_, embedding_dim_, forget_bias_, rnn_num_units):
        ...
        logits = tf.add(tf.matmul(state_output, weights),
...
# If want to check model with demo data uncomment below code :


# def checking_model(model):
#     with tf.Session() as sess:
#         sess.run(tf.global_variables_initializer())
#         out_now=sess.run(model.output,feed_dict={model.placeholder['input']:np.random.randint(0,10,[10,10]),model.placeholder['label_s']:np.random.randint(0,2,[10,]),model.placeholder['dropout_mode']:0})
#         print(out_now)

# if __name__=="__main__":

#     model=LSTM_Network(10,0.5,100,1.0,150)
